...
class user {
    ...
    private $conn;
    //db fileds
    protected static $db_fields = array('username', 'designation', 'login_id', 'password', 'email', 'phone', 'role_id', 'status');
...
public function __construct() {
    ...
    public function find_all() {
        if ($_SESSION['role'] == 1) {
            return $this->conn->query("SELECT * FROM " . static::$table_name." INNER JOIN roles ON users.role_id = roles.pk_id");
        } else {
            return $this->conn->query("SELECT * FROM " . static::$table_name ." INNER JOIN roles ON users.role_id = roles.pk_id WHERE status = 1");
        }
    }
